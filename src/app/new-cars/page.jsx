...
export const metadata = {
  title: "New Cars - Premium Collection",
};
...
const NewCarsPage = async ({ searchParams }) => {
  ...
  // 💡 শুধুমাত্র 'new' কন্ডিশন ফিল্টার
  let newCars = Array.isArray(allCars) 
    ? allCars.filter(car => car.condition?.toLowerCase() === 'new')
    : [];

  // সর্টিং লজিক
  if (currentSort === 'low-to-high') {
    newCars = [...newCars].sort((a, b) => (a.price || 0) - (b.price || 0));
  } else if (currentSort === 'high-to-low') {
    newCars = [...newCars].sort((a, b) => (b.price || 0) - (a.price || 0));
  } else if (currentSort === 'default') {
    newCars = [...newCars].sort((a, b) => (a.title || '').localeCompare(b.title || ''));
  }

  const displayedCars = newCars.slice(0, currentLimit);
    ...
    <div className="w-full bg-[#F9F9F9] min-h-screen font-sans antialiased text-[#111111]">
      ...
      >
        ...
        <div className="relative z-10 text-center">
          <h1 className="text-3xl md:text-5xl font-bold tracking-wide mb-3 uppercase">Brand New Cars</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-center">
          ...
          <div className="lg:col-span-1">
            <button className="w-full flex items-center justify-between bg-white border border-gray-200 p-4 font-bold text-[#111111] text-sm uppercase tracking-wider group">
              <span className="flex items-center gap-3">Find Your Dream Car</span>
